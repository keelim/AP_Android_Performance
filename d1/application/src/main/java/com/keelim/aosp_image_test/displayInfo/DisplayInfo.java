 ...
 * @hide
 */
public final class DisplayInfo implements Parcelable, Cloneable {
    /**
    ...
    public DisplayInfo copyFrom(DisplayInfo other) {
        try {
            return other.clone();
        } catch (CloneNotSupportedException e) {
            e.printStackTrace();
            return new DisplayInfo(other);
        }

    }

    @Override
    protected DisplayInfo clone() throws CloneNotSupportedException {
        DisplayInfo di = (DisplayInfo) super.clone();
        di.supportedModes = Arrays.copyOf(this.supportedModes, this.supportedModes.length);
        di.supportedColorModes = Arrays.copyOf(
                this.supportedColorModes, this.supportedColorModes.length);
        di.address = this.address;
        di.displayCutout = this.displayCutout;
        di.hdrCapabilities = this.hdrCapabilities;
        return di;
    }

    public void copyFromOrigin(DisplayInfo other) {
        layerStack = other.layerStack;
        flags = other.flags;
        type = other.type;
        displayId = other.displayId;
